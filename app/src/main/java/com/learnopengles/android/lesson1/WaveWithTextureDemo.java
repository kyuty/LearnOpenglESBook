 ...
 */
public class WaveWithTextureDemo implements GLSurfaceView.Renderer
{
    /**
...
//        mTriangle1Vertices.put(triangle1VerticesData).position(0);
    ...
    private float z = 0;
    int numsVertices = (4 + (n -1) * 2);    // vertex nums
    float[] waveVertices = new float[numsVertices * 3 ];
    float[] waveTextures = new float[numsVertices* 2];
    short[] waveIndices = new short[n * 2 * 3]; // n*2个三角形
    ...
    private void initData() {
        ...
        // create texture point -- begin
        for(int i = 0; i < numsVertices; i++){
            waveTextures[i * 2 + 0] = waveVertices[i * 3 + 0];
            waveTextures[i * 2 + 1] = waveVertices[i * 3 + 1];
        }
        // create texture point -- end
